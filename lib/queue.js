...
exports = module.exports = {
  ...
  farm_out: function(job, cb){
    ...
    redis.on('message', function(channel, msg){
      console.log("Received "+channel+": "+msg)
      cb(msg)
      redis.end()
    })
  },
  ...
  json_respond: function(code, data, response){
    response.writeHead(code, {'Content-Type': 'application/json' });
    var msg = JSON.stringify(data)
    response.end(msg);

    console.log("response HTTP "+code+": "+msg)
  },

  success_respond: function(response_msg, response){
    answer = edn.parse(response_msg)
    result = { jsonrpc: "2.0",
               id: answer.at('id'),
               result: edn.toJS(answer.at('result'))}
    this.json_respond(200, result, response)
  },

  error_respond: function(response){
    result = { error: "error"}
    json_respond(200, result, response)
  },
